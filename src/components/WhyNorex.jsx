...
function WhyNorex({ lang }) {
  ...
  return (
    <section id="why" dir={isAr ? "rtl" : "ltr"} className="relative py-16 md:py-24 px-6 md:px-12 bg-[#112819] overflow-hidden section-glow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10">
          {pillars.map((pillar, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              whileHover={{ y: -12 }}
              className={`pillar-card relative p-8 md:p-12 rounded-[2.5rem] border border-white/10 hover:border-[#00F299]/70 bg-[#0D2016] hover:bg-[#0a1c12] transition-all duration-500 flex flex-col h-full group ${
                isAr ? 'text-right' : 'text-left'
              }`}
            >
              <div className={`absolute -top-6 text-[110px] md:text-[140px] font-black text-[#00F299]/10 group-hover:text-[#00F299]/20 transition-all duration-500 select-none ${
                isAr ? 'left-6' : 'right-6'
              }`}>
                {pillar.number}
              </div>
              ...
              {/* Title */}
              <h3 className="text-2xl md:text-3xl font-black mb-6 text-white group-hover:text-[#00F299] transition-colors leading-tight z-10">
                {pillar.title}
              </h3>

              {/* Description */}
              <p className="text-white/80 text-base md:text-lg leading-relaxed flex-1 z-10">
                {pillar.desc}
              </p>
            </motion.div>
          ))}
